...
export class ImageUpdate extends plugin {
    ...
    async cloneRepo() {
        ...
        try {
            if (fs.existsSync(repoPath)) {
                return { 
                    success: false, 
                    message: '仓库已存在，无需重新安装，可使用更新命令', 
                    fileCount: 0 
                };
            }
            
            await this.e.reply(`首次安装，开始克隆${branch}分支仓库【可能需要几分钟...】`);
            ...
            if (result.success && result.fileCount === 0) {
                return {
                    success: false,
                    message: '克隆成功但未获取到任何文件，可能仓库为空或引用错误',
                    fileCount: 0,
    ...
    async pullRepo(force = false) {
        ...
        try {
            if (!fs.existsSync(repoPath)) {
                return { 
                    success: false, 
                    message: '仓库不存在，请先使用安装命令', 
                    fileCount: 0 
                };
            }

            if (force) {
                await this.e.reply(`检测到已有仓库，执行强制更新${branch}分支【正在重置本地修改...】`);
    ...
    analyzeErrorReason(message) {
        if (message.includes('remote HEAD refers to nonexistent ref')) {
            return '仓库引用错误，master分支不存在或URL不正确';
        }
    ...
    async installImage() {
        ...
        await this.e.reply('开始尝试安装不羡仙图像资源，请耐心等待~');
        
        const result = await this.cloneRepo();

        if (result.success) {
            if (result.fileCount > 0) {
                await this.e.reply(`图片资源安装成功~共下载了${result.fileCount}个图片！`);
                await this.e.reply('后续可通过 #不羡仙图像更新 命令获取最新资源');
            } else {
                await this.e.reply('安装过程完成，但未检测到任何文件');
            }
        } else {
    ...
    async updateImage() {
        ...
        await this.e.reply('开始尝试更新不羡仙图像资源，请耐心等待~');
        ...
        if (result.success) {
            if (result.fileCount > 0) {
                await this.e.reply(`图片资源更新成功~此次更新了${result.fileCount}个图片~`);
                await this.e.reply('后续可通过 #不羡仙图像更新 命令再次更新');
            } else {
                await this.e.reply('不羡仙图像已是最新版，暂无更新内容~');
            }
        } else {
            const reason = this.analyzeErrorReason(result.message);
            const tip = `更新失败！【${reason}】\n${isForce ? '如强制更新仍失败，请检查仓库地址或网络' : '建议使用「#不羡仙图像强制更新」重试'}`;

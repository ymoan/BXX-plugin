...
const Render = {
  async render(path, params, cfg = { retType: 'default', saveId: '' }) {
    ...
    return e.runtime.render('BXX-plugin', path, params, {
      retType: cfg.retType,
      beforeRender({ data }) {
        let pluginName = ''
        if (data.pluginName !== false) {
          pluginName = ` & ${data.pluginName || 'BXX-plugin'}`
          if (data.pluginVersion !== false) {
            pluginName += `<span class="version">${data.pluginVersion || Version.version}`
          }
        }
        let resPath = data.pluResPath
        const layoutPath = process.cwd() + '/plugins/BXX-plugin/resources/common/layout/'
        ...
        return {
          ...
          _tpl_path: process.cwd() + '/plugins/BXX-plugin/resources/common/tpl/',
